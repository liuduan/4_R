# setwd("C:\\Users\\FGrimm\\Desktop\\Total Cells - 90 min") #####Set Working Directory - Make sure to use double 
setwd("C:\\4_R\\R-from-Fabian") 
if(file.exists(".\\Figs\\TC_90min1.png")){
	a = 4
	quit()}
# backslash # !#####
...
w2=read.csv("Data.csv") #####Select Data File#####
dmso=read.csv("Vehicle.csv")#####Select Vehicle Data File#####
...
conc=c(rep(0.01,length(controls)),rep(0.1,1),rep(1,1),rep(10,1),rep(100,1)) #####Specify Concentration-Response#####

startcol=2 #####Columns in Data.csv#####
endcol=5 #####Columns in Data.csv#####
...
# pdf(".\\Figs\\TC_90min.pdf",height=5,width=5) ##### PDF Output#####
  
  for (i in (1:numchem)){ 
    file_name = paste(".\\Figs\\TC_90min", i, ".png", sep="")
    png(file_name) 
    ...
    if (sd(noncontrols)>0){
      ...
      result=together.logit4(x,y,plot.it=T,threshold=-25) ## plot is here.
...
write.csv(final.notlog10.df,"TC_90min_originalscale.csv") #####File Name for csv Output#####
